...
<?php
  ...
  $package_name = getPackageName($pid);
   ...
   echo "<tr>";
   echo "<td bgcolor='#084B8A'><a href='EventIndividualPackagePosting.php?pid=$pid'>INDIVIDUAL PACKAGE POSTING</a></td>";
   echo "<td><a href='EventCompanyPackagePosting.php?pid=$pid'>COMPANY PACKAGE POSTING</a></td>";
   echo "</tr>";
   echo "</table></br>";
  ...
  $display = "<table id='billings' width='100%' align='center' id='packages'>"
           . "<thead>"
   ...
   $all_contacts = array();
    ...
    foreach($bills as $key=>$field){
         ...
         if($post_bill == 0 && $field['total_amount'] != 0){
